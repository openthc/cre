...
class Simple extends \OpenTHC\Middleware\Base
{
	public function __invoke($REQ, $RES, $NMW)
	{
		...
		if (empty($_POST['service'])) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Parameter "service" missing [MAS-016]' ]
			], 400);
		}

		if (empty($_POST['company'])) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Parameter "company" missing [MAS-022]' ]
			], 400);
		}

		if (empty($_POST['license'])) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Parameter "license" missing [MAS-028]' ]
			], 400);
		}
		...
		if (empty($service_id)) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Invalid "service" [MAS-039]' ]
			], 403);
		}
		...
		if (empty($company_id)) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Invalid "company" [MAS-050]' ]
			], 403);
		}
		...
		if (empty($L['id'])) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Invalid "license" [MAS-061]' ]
			], 403);
		}
		...
		if ($company_id != $L['company_id']) {
			return $RES->withJSON([
				'meta' => [ 'note' => 'Invalid "license" [MAS-068]' ]
			], 403);
		}
