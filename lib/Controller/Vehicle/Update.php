...
class Update extends \OpenTHC\CRE\Controller\Base
{
	function __invoke($REQ, $RES, $ARG)
	{
		...
		$sql = 'SELECT id, meta FROM vehicle WHERE license_id = :l AND id = :g';
		$arg = array(
		...
		if (empty($chk)) {
			return $this->send404('Vehicle Not Found [CSU-023]');
		}
		...
		// Update the record
		$sql = 'UPDATE vehicle SET name = :n0, hash = :h, meta = :m WHERE id = :g';
		$arg = array(
			':g' => $ARG['id'],
			':n0' => trim($_POST['name']),
			':h' => sha1($meta),
		...
		// Log the change
		$this->logAudit('Vehicle/Update', $ARG['id'], json_encode($_POST));
